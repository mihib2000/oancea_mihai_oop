...
void UI::openbrowser(vector<Movie> aux, vector<Movie> &watchlist)
{
	...
	{
		cout << aux[i] << '\n';
		cout << "Vizualizeaza trailerul care se va deschide in browser \n";
...
void UI::user(vector<Movie>& my_list)
{
	...
	while (true)
	{
		...
		switch (option)
		{
			...
			case 4:
			{
				cout << "\nAlege modul de export: CSV, HTML, TXT (Scrieti: \"CSV\" sau \"HTML\" sau \"TXT\" si apasati enter)\n";
				string opt;
				cin >> opt;
				...
				if (opt == "CSV") {
					...
					contr.mapexports["TXT"]->write_filmslist_in_file(my_list, "Moviesw.txt"); // desigur, se putea si cu parametrii impliciti
